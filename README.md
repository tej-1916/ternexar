...
## v1.0: The Stable Safe Operator

Release v1.0 establishes the foundation for secure terminal automation. 

- **Verified Execution:** Safely execute LOW-risk commands (e.g., `ls`, `git status`) via a strict allowlist.
- **Robust Guardrails:** Blocks dangerous commands (e.g., `rm -rf /`, `cat .env`) and prevents shell-injection via character sanitization.
- **Full Observability:** Track every action with `tx audit`.
- **Pre-Flight Diagnostics:** Ensure your environment is ready with `tx doctor`.
